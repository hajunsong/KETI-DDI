...
from robot_a0509_master import Robot_Master_a0509
from robot_m1013_master import Robot_Master_m1013
...
class robot_master:
    ...
    def init_robot(self, robot_name = ''):
        ...
        if(robot_name == 'm1013'):
            #취향것 둘다 작동함
            self.robot_module[0] = Robot_Master_m1013()
            #self.robot_module[0] = Parent_Robot()
            #robot_slave = Robot_Module_m1013()
            #self.robot_module[0].m_Robot_Slave.mapping_function(robot_slave)
        if(robot_name == 'a0509'):
            #취향것 둘다 작동함
            self.robot_module[0] = Robot_Master_a0509()
            #self.robot_module[0] = Parent_Robot()
            #robot_slave = Robot_Module_m1013()
            #self.robot_module[0].m_Robot_Slave.mapping_function(robot_slave)
...
if __name__ == '__main__':
    ...
    # p1 = [180.0, 0.0, -90.0, 180.0, 90.0, 0.0]
    # m_robot_master.move_joint_robot(p1, vel=20, acc=20, syncType=1)

    pose = [450.0, 50, 440.5, 35.434, -180, 35.434]
    pose_init = [450.0, 0, 440.5, 35.434, -180, 35.434]
    pose_end =  [565.0, 195, 328.0, 35.434, -180, 35.434]
    ...
    while True:
        ...
        # target = [float(target_file_split[1]), float(target_file_split[2]), float(target_file_split[3]), (180-float(target_file_split[4])), int(target_file_split[5])]
        # # print target
        # print(target_file_split[5])
        # print(target[4])
        # state = int(target[4])
        # rot_y = pose_init[4]
        # if state == 1:
        ...
        # pose_goal = [target[0], target[1], pose_init[2], target[3]/2, rot_y, -target[3]/2]
        # m_robot_master.move_pos_robot(pose_goal, vel=[150,150], acc= [75,75], syncType = 1, time=2)
        ...
        # pose_goal = [target[0], target[1], target[2], target[3]/2, rot_y, -target[3]/2]
        # m_robot_master.move_pos_robot(pose_goal, vel=[80,160], acc= [40,100], syncType = 1, time=2)
        ...
        # # place 위치로 이동    
        # pose_goal = [target[0], target[1], pose_init[2], target[3]/2, rot_y, -target[3]/2]
        pose_goal = [947.45, 188.49, 317.41, 76.76, -146.94, -4.77]
        m_robot_master.move_pos_robot(pose_goal, vel=[150,150], acc=[75,75], syncType = 1, time=5)

        time.sleep(1)

        # pose_goal = [pose_end[0], pose_end[1], pose_init[2], pose_end[3], pose_end[4], pose_end[5]]
        pose_goal1 = [947.45, 188.49, 317.41, 76.76, -146.94, -4.77]
        m_robot_master.move_pos_robot(pose_goal1, vel=[80,160], acc=[40,100], syncType=1, time=2)
        ...
        pose_goal2 = [pose_goal1[0] + s[0], pose_goal1[1] + s[1], pose_goal1[2] + s[2], pose_goal1[3], pose_goal[4], pose_goal[5]]
        pose_list = [pose_goal1, pose_goal2]
        ...
        pose_goal = [pose_goal2[0], pose_goal2[1], pose_goal2[2] + 100, pose_goal2[3], pose_goal2[4], pose_goal2[5]]
        ...
        pose_goal = [pose_end[0], pose_end[1], pose_end[2], pose_end[3], pose_end[4], pose_end[5]]
        m_robot_master.move_pos_robot(pose_goal, vel=[80,160], acc=[40,100], syncType = 1)
