...
if __name__ == '__main__':
    ...
    pose_init = [300.0, 0, 440.5, 35.434, -180.0, 35.434]
    pose_end = [350.0, 550.0, 135.0, 35.434, -180.0, 35.434]
    ...
    m_robot_master.move_joint_robot(joint_init, syncType=1, time=5)
    print '로봇 초기 위치로 이동'
    ...
    while True:
        ...
        target = [float(target_file_split[1]), float(target_file_split[2]), float(target_file_split[3]), (180-float(target_file_split[4])), float(target_file_split[5]), float(target_file_split[4])]
        # print target_file_split
        # print target
        ###########################################################
        
        state = int(target_file_split[6])
        rot_y = pose_init[4]
        if state == 1:
            rot_y += 0
        elif state == 2:
        ...
        if target[2] < 190:
            temp = target[2]
            target[2] = 190
            print 'Z 최소 값 제한으로 값 변경 {0} => {1}'.format(temp, target[2])

        f_target_file_split = float(target_file_split[5]) * 1000 / 80.0
        openGripsSize = (1.0 - f_target_file_split) * (4000 - 100) + 100
        ...
        # target 위로 이동
        print 'target 위로 이동'
        pose_target_on = [target[0], target[1], pose_init[2], target[3]/2, rot_y, -target[3]/2]
        m_robot_master.move_pos_robot(pose_target_on, syncType=0, time=time_target_on)
        m_robot_master.gripper.grip_release()

        # target 가까이로 이동  
        print 'target 가까이로 이동'
        m_robot_master.gripper.grip(openGripsSize)
        pose_target_near = [target[0], target[1], target[2], target[3]/2, rot_y, -target[3]/2]
        m_robot_master.move_pos_robot(pose_target_near, syncType=1, time=time_target_near)

        # Tool 좌표계 기준 이동 - Doosan 제공
        ...
        # s = np.matmul(R, sp)
        #
        # pose_goal2 = [pose_goal1[0] + s[0], pose_goal1[1] + s[1], pose_goal1[2] + s[2], pose_goal1[3], pose_goal[4],
        #               pose_goal[5]]
        ...
        # home 위치로 이동
        print 'home 위치로 이동'
        joint_wp = [238.25, 12.36, -127.96, 179.03, 64.80, -33.30]
        joint_home_list = [joint_wp, joint_init]
        m_robot_master.move_joint_queue(joint_home_list, syncType=0, time=time_homming)
